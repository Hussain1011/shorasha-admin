...
class UserAuthController extends Controller
{
    public function register(Request $request){
        ...
        $user = User::where('id',$inserted)->first();
        ...
        $user = DB::table('users')->where('mobile', $request->mobile)->get()->map(function($record){
            // $record->url = ''.url("/").'/public/'.$record->photo.'';
            $record->url = str_replace('public','storage', $record->photo);
            return $record;
        });
        return response()->json([
            'status' => 'success',
    ...
    public function login(Request $request){
        ...
        {
        $user = User::where('email',$request->email)->first();//->where('user_type', $request->user_type)->first();
        } else if(!empty($request->mobile) && empty($request->email))
        {
        ...
        ->get()->map(function($record){
            $record->photo = ''.url("/").'/public/'.str_replace('public','storage', $record->photo);
            $record->certificate = ''.url("/").'/public/'.str_replace('public','storage', $record->certificate);
            $record->passport_id = ''.url("/").'/public/'.str_replace('public','storage', $record->passport_id);
            $record->id_face_1 = ''.url("/").'/public/'.str_replace('public','storage', $record->id_face_1);
            $record->id_face_2 = ''.url("/").'/public/'.str_replace('public','storage', $record->id_face_2);
            $record->cv = ''.url("/").'/public/'.str_replace('public','storage', $record->cv);
            return $record;
        });
        if(!$user || !Hash::check($request['password'],$user->password)){
            return response()->json([
       ...
       if(!empty($request->mobile) && empty($request->email))
        {
            $user = DB::table('users')->where('mobile', $request->mobile)->get()->map(function($record){
                // $record->url = ''.url("/").'/public/'.$record->photo.'';
                $record->url = ''.url("/").'/public/'.str_replace('public','storage', $record->photo);
                return $record;
            });
        }   else if (empty($request->mobile) && !empty($request->email)) 
        {
            $user = DB::table('users')->where('email', $request->email)->get()->map(function($record){
                // $record->url = ''.url("/").'/public/'.$record->photo.'';
                $record->url = ''.url("/").'/public/'.str_replace('public','storage', $record->photo);
                return $record;
            });
        ...
        if($user[0]->user_type == '2'){
            ...
            return response()->json([
                ...
                'status' => 'success',
                'user' => is_null($user[0])? null : $user[0],
                'user_personal' => $user_personal[0] ?? null,
            ]);
    ...
    public function otp_confirmed(Request $req){
        ...
        if($req->otp_confirmed == 'true'){
            ...
            $notified = DB::table('user_notifications')->insert([
                ...
                "content_en" => 'Your Account Activated Success',
    ...
    //2. addDoctorPersonal: Will be sending "residency", "passport", "language", "accent", "consultant_categorie"
    public function addDoctorPersonal(Request $req){
        ...
        if($req->residency ==''){
            return response()->json([
                "status"=> 'error',
                'message_en' => 'Residency Is Required!',
                'message_ar' => 'الإقامة مطلوبة'
            ]);
        }
        if($req->passport ==''){
            return response()->json([
                "status"=> 'error',
                'message_en' => 'Passport Is Required!',
                'message_ar' => 'جواز السفر مطلوب'
            ]);
        }
        // if($req->language ==''){
        //     return response()->json([
        //         "status"=> 'error',
        //         'message_en' => 'Language Is Required!',
        //         'message_ar' => 'اللغة مطلوبة'
        //     ]);
        // }
        if($req->accent ==''){
            return response()->json([
                "status"=> 'error',
                'message_en' => 'Accent Is Required!',
                'message_ar' => 'الجنسية مطلوبة'
            ]);
        }
        if($req->category ==''){
            return response()->json([
                "status"=> 'error',
                'message_en' => 'Category Is Required!',
                'message_ar' => 'الجنسية مطلوبة'
            ]);
        }
    ///////////////////////////////////////////////////////////////
        ...
        if ($req->file('photo')) {
            $photo = $req->user_id.'-photo.'.$req->photo->extension();
            $photo_path = $req->photo->storeAs('public/photos/doctors', $photo);
            $photo_path = str_replace('public','storage', $photo_path);
        }

       # upload user id face 1 if have
        if ($req->file('id_face_1')) {
            $id_face_1 = $req->user_id.'-id-face-1.'.$req->id_face_1->extension();
            $id_face_1_path = $req->id_face_1->storeAs('public/photos/doctors/IDs', $id_face_1);
            $id_face_1_path = str_replace('public','storage', $id_face_1_path);
        }

        # upload user id face 2 if have
        if ($req->file('id_face_2')) {
            $id_face_2 = $req->user_id.'-id-face-2.'.$req->id_face_2->extension();
            $id_face_2_path = $req->id_face_2->storeAs('public/photos/doctors/IDs', $id_face_2);
            $id_face_2_path = str_replace('public','storage', $id_face_2_path);
        }

        # upload user cv
        if ($req->file('cv')) {
            $cv = $req->user_id.'-cv.'.$req->cv->extension();
            $cv_path = $req->cv->storeAs('public/photos/doctors/cv', $cv);
            $cv_path = str_replace('public','storage', $cv_path);
        }

        # upload user profile if have
        if ($req->file('profile')) {
            $profile = $req->user_id.'-profile.'.$req->profile->extension();
            $profile_path = $req->profile->storeAs('public/photos/doctors', $profile);
            $profile_path = str_replace('public','storage', $profile_path);
        }

        # upload user certificate
        if ($req->file('certificate')) {
            $certificate = $req->user_id.'-certificate.'.$req->certificate->extension();
            $certificate_path = $req->certificate->storeAs('public/photos/doctors/certificate', $certificate);
            $certificate_path = str_replace('public','storage', $certificate_path);
        }

        # upload user audio
        if ($req->file('intro_audio')) {
            $intro_audio = $req->user_id.'-intro_audio.'.$req->intro_audio->extension();
            $intro_audio_path = $req->intro_audio->storeAs('public/photos/doctors/intro_audio', $intro_audio);
            $intro_audio_path = str_replace('public','storage', $intro_audio_path);
        }
        # upload user video
        if ($req->file('intro_video')) {
            $intro_video = $req->user_id.'-intro_video.'.$req->intro_video->extension();
            $intro_video_path = $req->intro_video->storeAs('public/photos/doctors/intro_video', $intro_video);
            $intro_video_path = str_replace('public','storage', $intro_video_path);
        }
        # upload user text
        if($req->intro_text ==''){
            return response()->json([
                "status"=> 'error',
                'message_en' => 'Info Is Required!',
                'message_ar' => 'معلومات مطلوب'
            ]);
        }
        # check exist
        ...
        # updating
            ...
            'nationality_id' => $req->accent_id,
            'nationality_id' => $req->category_id,
            ...
            'photo' => (is_null($req->photo))? null : $photo_path,//asset('public/uploads/photo/doctors').'/'.$photo.'',
            'cv' => (is_null($req->cv))? null : $cv_path,//asset('public/uploads/cv/doctors').'/'.$cv.'',
            // 'profile' => (is_null($req->profile))? null : $profile_path,//asset('public/uploads/cv/doctors').'/'.$cv.'',
            'certificate' => (is_null($req->certificate))? null : $certificate_path, //asset('public/uploads/certificates/doctors').'/'.$certificate.'',
            'intro_audio' => $intro_audio_path, //asset('public/uploads/certificates/doctors').'/'.$intro_audio.'',
            'intro_video' => $intro_video_path, //asset('public/uploads/certificates/doctors').'/'.$intro_video.'',
            'fees' => $req->fees,
            'intro_text' => $req->intro_text,
            'bio' => $req->intro_text,
            'experience_yrs' => $req->experience_years,
            'id_face_1' => $id_face_1_path,
            'id_face_2' =>$id_face_2_path,
        ]);
            ...
            $inserted = DB::table('user_personal')->insert([
                ...
                'passport' => $req->passport,
                'date_birth' =>$dateBirth,
                'photo' => (is_null($photo))? null : $photo_path,
                'certificate' => (is_null($certificate))? null : $certificate_path,
                'intro_audio' => (is_null($intro_audio))? null : $intro_audio_path,
                'intro_video' => (is_null($intro_video))? null : $intro_video_path,
                // 'cv' => (is_null($cv))? null : asset('public/uploads/cv/doctors').'/'.$cv.'',
                'cv' => (is_null($req->cv))? null : $cv_path,
                'fees' => $req->fees,
                'intro_text' => $req->intro_text,
                'bio' => $req->intro_text,
                'specialist_id' => $specialists[0],
                'id_face_1' => $id_face_1_path,
                'id_face_2' =>$id_face_2_path,
                // 'profile' => (is_null($profile))? null : $profile_path,
                'experience_yrs' => $req->experience_years,
    ...
    public function updateDoctorPersonal(Request $req){
        ...
        if($req->address !=''){
            # update address
            $updateCity = DB::table('user_personal')->where('user_id', $req->user_id)->update(['address' => $req->address]);
        }

        if($req->date_birth !=''){
            # update date_birth
            $updateCity = DB::table('user_personal')->where('user_id', $req->user_id)->update(['date_birth' => Carbon::create($req->date_birth)]);
        }
        ...
        if($req->intro_text !=''){
            //     # update city
                $updateCity = DB::table('user_personal')->where('user_id', $req->user_id)->update(['intro_text' => $req->intro_text]);
        }
        ...
        if($req->residency !=''){
            # update residency
            $updateCity = DB::table('user_personal')->where('user_id', $req->user_id)->update(['residency' => $req->residency]);
        }
        if($req->passport !=''){
            # update passport
            $updateCity = DB::table('user_personal')->where('user_id', $req->user_id)->update(['passport' => $req->passport]);
        }
        if($req->nationality_id !=''){
            # update city
            $updateCity = DB::table('user_personal')->where('user_id', $req->user_id)->update(['nationality_id' => $req->nationality_id]);
        ...
        $user_per = DB::table('user_personal')->where('user_id', Auth::user()->id)->first();
        # upload user profile if have
        if ($req->file('profile')) {
            $profile = $req->user_id.'-profile.'.$req->profile->extension();
            $profile_path = $req->profile->storeAs('public/photos/doctors', $profile);
        ...
        if ($req->file('id_face_1')) {
            ...
            $id_face_1_path = $user_per->id_face_1;
        }
        ...
        if ($req->file('id_face_2')) {
            ...
            $id_face_2_path = $user_per->id_face_2;
        }
        ...
        # upload user certificate
        if ($req->file('certificate')) {
            $certificate = $req->user_id.'-certifiacte.'.$req->certificate->extension();
            $certificate_path = $req->certificate->storeAs('public/photos/doctors/certificate', $certificate);
            $certificate_path = str_replace('public','storage', $certificate_path);
        }   else {
            $certificate_path = $user_per->certificate;
        }
        # upload user intro_audio
        if ($req->file('intro_audio')) {
            $intro_audio = $req->user_id.'-intro_audio.'.$req->intro_audio->extension();
            $intro_audio_path = $req->intro_audio->storeAs('public/photos/doctors/intro_audio', $intro_audio);
            $intro_audio_path = str_replace('public','storage', $intro_audio_path);
        }   else {
            $intro_audio_path = $user_per->intro_audio;
        }
        # upload user intro_video
        if ($req->file('intro_video')) {
            $intro_video = $req->user_id.'-intro_video.'.$req->intro_video->extension();
            $intro_video_path = $req->intro_video->storeAs('public/photos/doctors/intro_video', $intro_video);
            $intro_video_path = str_replace('public','storage', $intro_video_path);
        }   else {
            $intro_video_path = $user_per->intro_video;
        }
        # check exist
        $count = DB::table('user_personal')->where('user_id', Auth::user()->id)->count();
        ...
        if($count > 0){
            ...
            $updated = DB::table('user_personal')->where('user_id', Auth::user()->id)->update([
                ...
                // 'passport_id' => 'uploads/passports').'/'.$passport.'',
            ]);
            ...
            $inserted = DB::table('user_personal')->insert([
                ...
                'intro_video' => (is_null($intro_video))? null : $intro_video_path,
                'intro_audio' => (is_null($intro_audio))? null : $intro_audio_path,
                // 'passport_id' => (is_null($passport))? null : asset('uploads/passports').'/'.$passport.'',
                'profile' => (is_null($profile))? null : $profile_path,
                'fees' => $req->fees,
                'specialist_id' => $specialists[0],
                'created' => $created->getTimestampMs()
    ...
    public function specialists(Request $req){
        $data = DB::table('specialists')->where('active', 1)->get()->map(function($record){
            $record->photo = ''.url("/").'/public/'.$record->photo.'';
            // $record->photo = url("/").str_replace('public','storage', $record->photo);
            return $record;
        });
        return response()->json([
            "status"=> 'success',
    ...
    public function selectionData(Request $req){
        ...
        $data['languages'] = Language::with('accents')->get();
        return response()->json([
            "status"=> 'success',
            // 'message_en' => 'Inserted Success',
                'data' => $data
    ...
    public function listDoctors(Request $req){
        ...
        ->get()->map(function ($record){
                # images url for app
                // $record->photo = ''.url("/").'/public/'.$record->photo.'';
                $record->photo = str_replace('public','storage', $record->photo);
                // $record->certificate = ''.url("/").'/public/'.$record->certificate.'';
                $record->certificate = str_replace('public','storage', $record->certificate);
                // $record->passport_id = ''.url("/").'/public/'.$record->passport_id.'';
                $record->passport_id = str_replace('public','storage', $record->passport_id);
                // $record->id_face_1 = ''.url("/").'/public/'.$record->id_face_1.'';
                $record->id_face_1 = str_replace('public','storage', $record->id_face_1);
                // $record->id_face_2 = ''.url("/").'/public/'.$record->id_face_2.'';
                $record->id_face_2 = str_replace('public','storage', $record->id_face_2);
                // $record->cv = ''.url("/").'/public/'.$record->cv.'';
                $record->cv = str_replace('public','storage', $record->cv);
                # doctor rating
        ...
        ->get()
            ->map(function ($record){
                ...
                $data['doctorSpecialists'] = DB::table('doctor_specialists')
                    ...
                    ->where('doctor_specialists.active', 1)
                    ->where('doctor_specialists.doctor_id', $record->user_id)
                    ->select('specialists.title_en','specialists.title_ar','more_info_title_en','more_info_title_ar','more_info_en','more_info_ar')
                    ->get();
                $record->doctorSpecialists = $data['doctorSpecialists'];
    ...
    public function popularDoctors(Request $req){
        ...
        ->get()
            ->map(function ($record){
                ...
                $record->photo = ''.url("/").'/public/'.str_replace('public','storage', $record->photo);
                // $record->certificate = ''.url("/").$record->certificate.'';
                $record->certificate = ''.url("/").'/public/'.str_replace('public','storage', $record->certificate);
                // $record->id_face_1 = ''.url("/").$record->id_face_1.'';
                $record->id_face_1 = ''.url("/").'/public/'.str_replace('public','storage', $record->id_face_1);
                // $record->id_face_2 = ''.url("/").$record->id_face_2.'';
                $record->id_face_2 = ''.url("/").'/public/'.str_replace('public','storage', $record->id_face_2);
                // $record->cv = ''.url("/").$record->cv.'';
                $record->cv = ''.url("/").'/public/'.str_replace('public','storage', $record->cv);
                # doctor rating
                $rates = DB::table('users_reviews')->where('doctor_id', $record->user_id)->sum('rate');
                $totalRates = DB::table('users_reviews')->where('doctor_id', $record->user_id)->count();
                ...
                if(count($bookings) > 0){
                    foreach ($bookings as $key => $booking) {
                        if($booking->schedule_code == $doctorSchedule->created && ($booking->booking_status == 1 || $booking->booking_status == 0)){
                            // $date1 = Carbon::parse($todayDateTime);
                            $todayDateTime = Carbon::now();
                            $date2 = Carbon::create($booking->created_at);
                            $interval = $date2->diffInMinutes($todayDateTime);
                            if($interval<10 || $booking->paid==1)
                            {
    ...
    public function addDoctorCertificate(Request $req){
        ...
        foreach ($req->file as $key => $value) {
            ...
            $certificate = $req->doctor_id.'certificate-'.$key.'.'.$req->certificate->extension();
            $certificate_path = $req->certificate->storeAs('public/photos/doctors/certificate', $certificate);
            $certificate_path = str_replace('public','storage', $certificate_path);
    ...
    public function updateDoctorDocuments(Request $req){
      ...
      $intro_audio_path = '';
        ...
        //     $req->photo->move(public_path('uploads/photos/doctors/phd/'), $photo);
        // }
        $data = DB::table('user_personal')->where('user_id', Auth::user()->id)->first();
         
        if ($req->file('phd')) {
            $phd = $req->user_id.'-phd.'.$req->phd->extension();
            $phd_path = $req->phd->storeAs('public/photos/doctors/phd', $phd);
            $phd_path = str_replace('public','storage', $phd_path);
        }  
        // else{
        //     $phd_path = $data->phd;
        // }
        ...
        //     $req->certificate->move(public_path('uploads/photos/doctors/certificate/'), $certificates);
        // }
        ...
        if ($req->file('intro_audio')) {
            ...
            $intro_audio_path = $data->intro_audio;
        }
        ...
        if ($req->file('intro_video')) {
            ...
            $intro_video_path = $data->intro_video;
        }
    ...
    public function userBookings(Request $req){
        ...
        ->get()->map(function($record){
            ...
            // $record->certificate = ''.url("/").''.$record->certificate.'';
            // $record->certificate = str_replace('public','storage', $record->certificate);
            // // $record->passport_id = ''.url("/").''.$record->passport_id.'';
            // $record->passport_id = str_replace('public','storage', $record->passport_id);
            // // $record->id_face_1 = ''.url("/").''.$record->id_face_1.'';
            // $record->id_face_1 = str_replace('public','storage', $record->id_face_1);
            // // $record->id_face_2 = ''.url("/").''.$record->id_face_2.'';
    ...
    public function doctorCompletedSession(Request $req){
        # check if have already session
        # update status
        $udated = DB::table('appointments_booking')
    ...
    public function changeConsultantStatus(Request $req)
    {
        ...
        if(is_null($req->status)){
            return response()->json([
                ...
                'message_en' => 'Status is Required'
            ]);
        }

        $updated = DB::table('user_personal')
        ->where('user_id', $req->user_id)
        ->update([
            'status' => $req->status,
        ]);
    ...
    public function userAdvs(Request $req){
        # get all banners
        $advs = DB::table('advs')->where('type', 'user')->where('active',1)->get();
        return response()->json([
            'status' => 'success',
            'data'=>$advs
        ]);
    ...
    public function doctorAdvs(Request $req){
        # get all advs
        $advs = DB::table('advs')->where('type', 'doctor')->where('active',1)->get();
        return response()->json([
            'status' => 'success',
            'data'=>$advs
        ]);
    ...
    public function doctorHomePage(Request $req){ 
        ...
        $data['doctorSpecialists'] = DB::table('specialists')
            ...
            ->where('doctor_specialists.doctor_id', Auth::user()->id)->get()->map(function($record){
                // $record->photo = ''.url("/").'/public/'.$record->photo;
                $record->photo = str_replace('public','storage', $record->photo);
                return $record;
            });
    ...
    public function userUpdate(Request $req){
        // dd($req);
        # get auth email
        $currEmail = DB::table('users')->where('id', $req->user_id)->select('email','user_type','photo')->first();
        ...
        //     // dd(url('/'));
        //     $url = '/public/uploads/photos/users/'.$photo.'';
        //     # update photo db
        ...
        if ($req->file('photo')) {
            $photo = $req->user_id.'-photo.'.$req->photo->extension();
            if($currEmail->user_type == 1){ // user
                $photo_path = $req->photo->storeAs('public/photos/users', $photo);
            }else{ // doctor
                $photo_path = $req->photo->storeAs('public/photos/doctors', $photo);
            }
            $photo_path = str_replace('public','storage', $photo_path);
            ...
            # update photo db
            $updated = DB::table('users')->where('id', $req->user_id)->update(['photo' => $photo_path]);
            $updated = DB::table('user_personal')->where('user_id', $req->user_id)->update(['photo' => $photo_path]);
      
        }else{
            $photo_path = $currEmail->photo;
        }
 
        return response()->json([
            'status' => 'success',
            'message_ar' => 'تم التحديث بنجاح',
            'photo' => ''.url("/").'/public/'.$photo_path,
            'message_en' => 'Updated Success'
        ]);
    ...
    public function doctorPhotoUpdate(Request $req){
        ...
        //     // dd(url('/'));
        //     $url = '/public/uploads/cv/doctors/'.$photo.'';
        //     # update photo db
        ...
        # upload user photo if have
        if ($req->file('photo')) {
            $photo = $req->doctor_id.'-photo.'.$req->photo->extension();
            $photo_path = $req->photo->storeAs('public/photos/doctors', $photo);
            $photo_path = str_replace('public','storage', $photo_path);
            # update photo db
            $updated = DB::table('user_personal')->where('user_id', $req->doctor_id)->update(['photo' => $photo_path]);
        }
    ...
    public function updateID(Request $req){
        ...
        //     // dd(url('/'));
        //     $url = '/public/uploads/IDs/'.'id_face_1-'.$photo.'';
        //     # update photo db
        //     $updated = DB::table('user_personal')->where('user_id', $req->doctor_id)->update(['id_face_1' => $url]);
        // }

        if ($req->file('id_face_1')) {
            $id_face_1 = $req->doctor_id.'-id-face-1.'.$req->id_face_1->extension();
            $id_face_1_path = $req->id_face_1->storeAs('public/photos/doctors/IDs', $id_face_1);
            $id_face_1_path = str_replace('public','storage', $id_face_1_path);

            # update photo db
            $updated = DB::table('user_personal')->where('user_id', $req->doctor_id)->update(['id_face_1' => $id_face_1_path]);
        }
        ...
        //     // dd(url('/'));
        //     $url = '/public/uploads/IDs/'.'id_face_2-'.$photo.'';
        //     # update photo db
        //     $updated = DB::table('user_personal')->where('user_id', $req->doctor_id)->update(['id_face_2' => $url]);
        // }

        if ($req->file('id_face_2')) {
            $id_face_2 = $req->doctor_id.'-id-face-2.'.$req->id_face_2->extension();
            $id_face_2_path = $req->id_face_2->storeAs('public/photos/doctors/IDs', $id_face_2);
            $id_face_2_path = str_replace('public','storage', $id_face_2_path);
    ...
    public function userNotificationOpened(Request $req){
        DB::table('user_notifications')->where('user_id', $req->user_id)->where('created', $req->id)->update(['read' => 1]);
        return response()->json([
            'status' => 'success'
        ]);
    }
    ...
    public function checkBookingAvailableTest(Request $req){
        ...
        if($exist > 0){
        $app_book = DB::table('appointments_booking')
        ->where('doctor_id', $req->doctor_id)
        ->where('schedule_code', $req->created)
        ->where('day_index', $req->day_index)
        ->where('date', ''.$req->date.'')
        ...
        $todayDateTime = Carbon::now();
        $date2 = Carbon::create($app_book->created_at);
        $interval = $date2->diffInMinutes($todayDateTime);
        
            if($interval<10 || $app_book->paid==1)
            {
    ...
    public function doctorInfo(Request $request){
        # doctor details
        // $user = DB::table('users')->join('user_personal','user_personal.user_id','=','users.id')
        // ->select('users.*','user_personal.*')
    ...
    public function scheduleGenerate(Request $req){
        ...
        for ($i= 0; $i <= $times ; $i++) {
            ...
            ->insert([
                ...
                'to_hour' => $newStart->addMinutes(60)->toTimeString(),
                'created_by' => $req->doctor_id,
