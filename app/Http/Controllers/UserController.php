...
*/
   ...
   public function consultantShow($id)
    ...
    public function medal_assign(Request $request)
    {
        // dd($request->all());
        $user = User::find($request->id);
        $user_personal = UserPersonal::where('user_id', $request->id)->first();
        $user_personal->medal = $request->medal;
        $user_personal->save();
        return back()->with('message', 'success');
    }
    ...
    public function action(Request $request)
    {
        ...
        if ($request->has('reason_profile')) {
        $reasons['profile'] = [
            'details' => $request->input('reason_profile_input')
        ];
        }
        if ($request->has('reason_id_front')) {
        $reasons['id_front'] = [
            'details' => $request->input('reason_id_front_input')
        ];
        }
        if ($request->has('reason_id_back')) {
        $reasons['id_back'] = [
            'details' => $request->input('reason_id_back_input')
        ];
        }
        if ($request->has('reason_photo')) {
        $reasons['photo'] = [
            'details' => $request->input('reason_photo_input')
        ];
        }
        if ($request->has('reason_certificate')) {
        $reasons['certificate'] = [
            'details' => $request->input('reason_certificate_input')
        ];
        }
        if ($request->has('reason_intro_text')) {
        $reasons['intro_text'] = [
            'details' => $request->input('reason_intro_text_input')
        ];
        }
        if ($request->has('reason_intro_video')) {
        $reasons['intro_video'] = [
            'details' => $request->input('reason_intro_video_input')
        ];
        }
        if ($request->has('reason_intro_audio')) {
        $reasons['intro_audio'] = [
            'details' => $request->input('reason_intro_audio_input')
        ];
        }
        if ($request->has('reason_address')) {
        $reasons['address'] = [
            'details' => $request->input('reason_address_input')
        ];
        }
        if ($request->has('reason_residency')) {
        $reasons['residency'] = [
            'details' => $request->input('reason_residency_input')
        ];
        }
        if ($request->has('reason_passport')) {
        $reasons['passport'] = [
            'details' => $request->input('reason_passport_input')
        ];
        }
        if ($request->has('reason_experience_yrs')) {
        $reasons['experience_yrs'] = [
            'details' => $request->input('reason_experience_yrs_input')
        ];
        }
        if ($request->has('reason_date_birth')) {
        $reasons['date_birth'] = [
            'details' => $request->input('reason_date_birth_input')
        ];
        }
        if ($user_personal->doctor_verified == 1)
        {
            $user_personal->rejection_reason = NULL;
        }
        if($request->doctor_verified == 1)
        {
            $reasons = NULL;
        }
        $user_personal->rejection_reason = $reasons;
        $user_personal->save();
        return back()->with('message', 'success');
    }
    ...
    public function consultantStore(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'intro_audio' => 'image|mimes:jpeg,png,jpg,gif,svg|max:2048',
            'intro_video' => 'image|mimes:jpeg,png,jpg,gif,svg|max:2048',
            'intro_text' => 'required',
            'speciality' => 'required',
        ...
        if ($request->file('doctor_id_1')) {
            ...
            $doctor_id_1_path = $request->doctor_id_1->storeAs('public/doctors/IDs', $doctor_id_1);
        ...
        if ($request->file('doctor_id_2')) {
            ...
            $doctor_id_2_path = $request->doctor_id_2->storeAs('public/doctors/IDs', $doctor_id_2);
        ...
        if ($request->file('intro_audio')) {
            // $file = $request->file('certificate');
            // $certificate_path = $file->store('uploads');

            $intro_audio = time().'-intro_audio.'.$request->intro_audio->extension();  
            $intro_audio_path = $request->intro_audio->storeAs('public/intro_audio', $intro_audio);
        ...
        if ($request->file('intro_video')) {
            ...
            $intro_video = time().'-intro_video.'.$request->intro_video->extension();  
            $intro_video_path = $request->intro_video->storeAs('public/intro_video', $intro_video);
        ...
        $user_personal->intro_audio = isset($intro_audio_path)? $intro_audio_path : null;
        $user_personal->intro_video = isset($intro_video_path)? $intro_video_path : null;
        $user_personal->fees = $request->fees;
        $user_personal->intro_text = $request->intro_text;
    ...
    public function consultantUpdate(Request $request)
    {
        ...
        if ($request->file('intro_video')) {
            ...
            $intro_video = time().'-intro_video.'.$request->intro_video->extension();  
            $intro_video_path = $request->certificate->storeAs('public/intro_video', $intro_video);
            $intro_video_path = str_replace('public','storage', $intro_video_path);
        }
