...
class BannersController extends Controller
{
    public function index(Request $request)
    {
        ...
        // $banners = User::where('user_type', 1)->when($request->search, function ($q) use ($request) {
    ...
    public function deletedUsers(Request $request)
    {
        ...
        return view('admin/user/banner-list')
            ->with('deleted',true)
            ->with('routePrefix', $routePrefix)
            ->with('users', $users);
    }
...
*/
   ...
   public function consultantShow($id)
    ...
    public function show($id)
    {
        ...
        $routePrefix = explode('/', $pageName)[0] ?? '';
        $user = User::where('id', $id)->first();
        return view('admin/user/banner-profile')
            ->with('routePrefix', $routePrefix)
            ->with('user', $user);
    }
    ...
    public function edit($id)
    {
        $pageName = request()->route()->getName();
        $routePrefix = explode('/', $pageName)[0] ?? '';
        $banner = DB::table('banner')->where('id', $id)->first(); //User::where('id', $id)->first();
    ...
    public function update(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'banner_ar' => 'required|image|mimes:jpeg,png,jpg,gif,svg|max:2048',
            'banner_en' => 'required|image|mimes:jpeg,png,jpg,gif,svg|max:2048'
        ]);
        ...
        $details = DB::table('banner')->where('id', $request->id)->first();
        $banner_en_title = explode('banners/', $details->image_en);
        $banner_en = $banner_en_title[1]; //time().'.'.$request->banner_en->extension();  
        $banner_en_path = $request->banner_en->storeAs('public/banners', $banner_en);
        $banner_en_path = str_replace('public','storage', $banner_en_path);

        $banner_ar_title = explode('banners/', $details->image_ar);
        // dd($banner_ar_title[1]);
        $banner_ar = $banner_ar_title[1]; //time().'.'.$request->banner_ar->extension();  
        $banner_ar_path = $request->banner_ar->storeAs('public/banners', $banner_ar);
        $banner_ar_path = str_replace('public','storage', $banner_ar_path);

        $updated = DB::table('banner')->where('id', $request->id)->update([
            'image_en' => $banner_en_path,
            'image_ar' => $banner_ar_path,
            'updated_at' => now()
        ]);
        return back()->with('message', 'success');
    }
    ...
    public function store(Request $request)
    {
        ...
        $banner_en = time().'-en.'.$request->banner_en->extension();  
        $banner_en_path = $request->banner_en->storeAs('public/banners', $banner_en);
        $banner_en_path = str_replace('public','storage', $banner_en_path);

        $banner_ar = time().'-ar.'.$request->banner_ar->extension();  
        $banner_ar_path = $request->banner_ar->storeAs('public/banners', $banner_ar);
        $banner_ar_path = str_replace('public','storage', $banner_ar_path);

        $inserted = DB::table('banner')->insert([
            'type' => $request->type,
            'image_en' => $banner_en_path,
            'image_ar' => $banner_ar_path,
            'active' => 1
    ...
    public function delete(Request $request)
    {
        ...
        $deleted = DB::table('banner')->where('id', $request->id)->delete();
        if($deleted){
