...
@section('content')
    ...
    {{-- <header class="bg-[#E8EBEB]">
        <!-- Border-Bottom Made with Before (to give it rounded effect) -->
        <a  href="{{ route('/consultant/list') }}"
        class="text-[#5B99A2] text-xl p-4 w-32" >{{trans('lang.current'))}}</a>
        <button href="{{ route('admin.consultant.index.new') }}"
        class="text-main text-xl p-4 w-32 relative before:content-[''] before:absolute before:bottom-0 before:start-0 before:h-[4px] before:w-full before:bg-main before:rounded-full"
        >{{trans('lang.new')}}</button>
    </header> --}}
    <header class="bg-[#E8EBEB] flex justify-between items-center">
        <div>
                <!-- Border-Bottom Made with Before (to give it rounded effect) -->
                <a href="{{ route('/consultant/list') }}" class="text-[#5B99A2] text-xl p-4 w-32">{{trans('lang.current')}}</a>
                <button href="{{ route('admin.consultant.index.new') }}"
                    class="text-main text-xl p-4 w-32 relative before:content-[''] before:absolute before:bottom-0 before:start-0 before:h-[4px] before:w-full before:bg-main before:rounded-full">{{trans('lang.new')}}</button>
                    <a href="{{ route('admin.consultant.index.deleted') }}" class="text-[#5B99A2] text-xl p-4 w-32">{{__('lang.deleted')}}</a>
                    <a href="{{ route('admin.consultant.change.list') }}"
                    class="text-[#5B99A2] text-xl p-4 w-32">{{ __('lang.change_request') }}</a>
                </div>
        <div class="flex gap-4 m-4">
            <a class="w-10 h-10 bg-main rounded-xl flex justify-center items-center" href="{{route('admin.consultant.create')}}">
        ...
        <div class="p-4 w-1/4">
            <div class="bg-white rounded-2xl shadow-[0px_2px_13px_0px_rgba(202,_229,_233,_0.48)]">
                <div class="p-6 flex justify-center items-center flex-col text-center gap-2">
                    <img class="w-20 h-20 rounded-full object-cover" src="{{url('/')}}/public/{{$user->personal?->photo}}" alt="consultant image">
