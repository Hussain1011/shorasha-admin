...
@section('content')
    <div class="bg-secondary w-full rounded-s-[2rem] p-6 flex flex-col gap-6">
        ...
        <!-- Edit Form -->
        <form action="{{ route('admin.banner.update',['id'=> $banner->id]) }}" method="POST" class="space-y-6" enctype="multipart/form-data">
            @csrf
            @method('post')
            ...
            <div class="bg-white shadow-[0px_2px_13px_0px_rgba(202,_229,_233,_0.048)] rounded-[1.375rem]">
                ...
                <div class="pt-16 pb-4 px-5 flex justify-between items-center border-b border-light-blue-200">
                    <p class="text-darkBlue font-semibold text-[1.375rem]">{{trans('lang.details')}}</p>
                </div>
                <div class="flex flex-col gap-4 p-6">
                    <div class="flex flex-col gap-4">
                        ...
                        <div class="flex">
                            ...
                            <!-- <img src="{{$banner->image_ar}}" width=200> -->
                        </div>
                        @error('banner_ar')
                            <span class="text-tiny+ text-error" style="color:red">{{ $message }}</span>
                        @enderror
                        ...
                        <div class="flex">
                            ...
                            <input type="file" id="image_en" name="banner_en" value="{{ old('image_en') }}"
                                class="py-3 px-4 rounded-[0.8125rem] bg-[#EAF8FA] text-darkGray border-none focus:ring-main focus:border-main block">
                                <!-- <img src="{{$banner->image_en}}" width=200> -->
                        </div>
                        @error('banner_en')
                            <span class="text-tiny+ text-error" style="color:red">{{ $message }}</span>
                        @enderror
            ...
            <div class="flex justify-end">
                <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">{{__('lang.save')}}
                </button>
            </div>
        </form>
    </div>
