    ...
    <div class="bg-secondary w-full rounded-s-[2rem] p-6 flex flex-col gap-6">
            ...
            <ol class="flex gap-1">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        <span class="text-main text-lg">{{trans('lang.edit')}} </span>
                    </div>
                </li>
            </ol>
        </nav>
        ...
        <form action="{{ route('admin.consultant.update') }}" method="POST" class="space-y-6" enctype="multipart/form-data">
                ...
                <div class="flex flex-col gap-4 p-6">
                    <div class="flex flex-col gap-4">
                        <div class="flex">
                            ...
                            <input type="text" id="name" name="name" value="{{ $details['user']->name }}"
                        ...
                        <div class="flex">
                            ...
                            <input disabled type="email" id="email" name="email" value="{{ $details['user']->email }}"
                            class="py-3 px-4 rounded-[0.8125rem] bg-[#EAF8FA] text-darkGray border-none focus:ring-main focus:border-main block w-full">
                        </div>
                        @error('email')
                            <span class="text-tiny+ text-error" style="color:red">{{ $message }}</span>
                        ...
                        <div class="flex">
                            ...
                            <input type="mobile" id="mobile" name="mobile" value="{{ $details['user']->mobile }}"
                                class="py-3 px-4 rounded-[0.8125rem] bg-[#EAF8FA] text-darkGray border-none focus:ring-main focus:border-main block w-full">
                        </div>
                        @error('mobile')
                            <span class="text-tiny+ text-error" style="color:red">{{ $message }}</span>
						...
						<div class="flex">
                            ...
                            <input type="fees" id="fees" name="fees" value="{{ $details['doctor']->fees }}"
                                class="py-3 px-4 rounded-[0.8125rem] bg-[#EAF8FA] text-darkGray border-none focus:ring-main focus:border-main block w-full">
                        </div>
                        @error('fees')
                            <span class="text-tiny+ text-error" style="color:red">{{ $message }}</span>
                        ...
                        <div class="flex">
                            ...
                            <input type="address" id="address" name="address" value="{{ $details['doctor']->address }}"
                                class="py-3 px-4 rounded-[0.8125rem] bg-[#EAF8FA] text-darkGray border-none focus:ring-main focus:border-main block w-full">
                        </div>
                        @error('address')
                            <span class="text-tiny+ text-error" style="color:red">{{ $message }}</span>
                        ...
                        <div class="flex">
                            ...
                            <select id="speciality" name="speciality[]" multiple class="py-3 px-4 rounded-[0.8125rem] bg-[#EAF8FA] text-darkGray border-none focus:ring-main focus:border-main block w-full">
                               ...
                               @foreach ($details['specialities'] as $index => $speciality)
                               @if($speciality->selected == 'yes')
                                <option selected value="{{ $speciality->id }}">{{ $speciality->title_en }}</option>
                                @else
                        ...
                        <div class="flex">
                            ...
                            <select id="country_id" name="country_id" class="py-3 px-4 rounded-[0.8125rem] bg-[#EAF8FA] text-darkGray border-none focus:ring-main focus:border-main block w-full">
                               ...
                               @foreach ($countries as $country)
                                @if($lan == 'ar')
                                @if($country->id == $details['doctor']->country_id)
                                ...
                                @else
                                @if($country->id == $details['doctor']->country_id)
                                <option selected value="{{ $country->id }}">{{ $country->title_en }}</option>
                                @else
                        ...
                        <div class="flex">
                            ...
                            <select id="city_id" name="city_id" class="py-3 px-4 rounded-[0.8125rem] bg-[#EAF8FA] text-darkGray border-none focus:ring-main focus:border-main block w-full">
                               ...
                               @foreach ($cities as $city)
                                @if($lan == 'ar')
                                @if($city->id == $details['doctor']->city_id)
                                <option selected value="{{ $city->id }}">{{ $city->title_ar }}</option>
                                @else
                                <option value="{{ $city->id }}">{{ $city->title_ar }}</option>
                                @endif
                                @else
                                @if($city->id == $details['doctor']->city_id)
                                <option selected value="{{ $city->id }}">{{ $city->title_en }}</option>
                                @else
                        ...
                        <div class="flex">
                            ...
                            <select id="nationality_id" name="nationality_id" class="py-3 px-4 rounded-[0.8125rem] bg-[#EAF8FA] text-darkGray border-none focus:ring-main focus:border-main block w-full">
                               ...
                               @foreach ($nationalities as $nationality)
                                @if($lan == 'ar')
                                @if($nationality->id == $details['doctor']->nationality_id)
                                <option selected value="{{ $nationality->id }}">{{ $nationality->title_ar }}</option>
                                @else
                                ...
                                @else
                                @if($nationality->id == $details['doctor']->nationality_id)
                                <option selected value="{{ $nationality->id }}">{{ $nationality->title_en }}</option>
                                @else
                        ...
                        @enderror
						<div class="flex">
                            <label for="intro_text" class="block w-1/4 text-[#808080]">{{trans('lang.intro_text')}}</label>
                            <textarea id="fees" name="fees" value="{{ $details['doctor']->intro_text }}"
                                class="py-3 px-4 rounded-[0.8125rem] bg-[#EAF8FA] text-darkGray border-none focus:ring-main focus:border-main block w-full">
                            </textarea>
                        </div>
                        @error('fees')
                        ...
                        <div class="flex">
                            <label for="id_expired_date" class="block w-1/4 text-[#808080]">{{trans('lang.expired_date')}}</label>
                            <input type="date" id="id_expired_date" name="id_expired_date" value="{{ $details['doctor']->id_expired_date }}"
                        ...
                        <div class="flex">
                            <label for="intro_audio" class="block w-1/4 text-[#808080]">{{ __('lang.intro_audio') }}</label>
                            <input type="file" id="intro_audio" name="intro_audio" class="py-3 px-4 rounded-[0.8125rem] bg-[#EAF8FA] text-darkGray border-none focus:ring-main focus:border-main block w-full">
                        </div>
                        @error('intro_audio')
                         <span class="text-tiny+ text-error" style="color:red">{{ $message }}</span>
                        @enderror
                        ...
                        <div class="flex">
                            <label for="intro_video" class="block w-1/4 text-[#808080]">{{ __('lang.intro_video') }}</label>
                            <input type="file" id="intro_video" name="intro_video" class="py-3 px-4 rounded-[0.8125rem] bg-[#EAF8FA] text-darkGray border-none focus:ring-main focus:border-main block w-full">
                        </div>
                        @error('intro_video')
                         <span class="text-tiny+ text-error" style="color:red">{{ $message }}</span>
                        @enderror
            ...
            <div class="flex justify-end">
                <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">{{trans('lang.save')}}</button>
            </div>
        </form>
    </div>
@endsection
