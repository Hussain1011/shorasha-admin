    ...
    <div>
        <div class="bg-white shadow-[0px_2px_13px_0px_rgba(202,_229,_233,_0.48)] rounded-2xl">
            <div class="flex items-center justify-between px-5 py-4">
                <span class="text-darkBlue text-[1.3125rem]">{{ trans('lang.accent') }}</span>
                <div class="flex items-center gap-4">
                    <a class="w-10 h-10 bg-main rounded-xl flex justify-center items-center" href="{{ route('admin.accent.create')  }}">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-start">
                    ...
                    <tbody>
                        @foreach ($accent as $row)
                        
                            <tr class="text-[#363636] hover:bg-[#EFF8FA]">
                                <td class="border-b border-light-blue-200 px-5 py-4">{{ $row->id }}</td>
                                <td class="border-b border-light-blue-200 px-5 py-4">{{ $row->language->title_en }}</td>
                                ...
                                <td class="border-b border-light-blue-200 px-5 py-4">
                                    <div class="flex gap-3 items-center">
                                        <a href="{{ url("admin/accent/edit/{$row->id}") }}">
                                            <img src="{{asset('images/images/actions-edit.svg')}}"
                                            alt="actions edit icon">

                                        </a>
                                        <form id="{{$row->id}}" action="{{ route('accent/delete', ['id' => $row->id]) }}"
                                            method="POST">
                                            @csrf
                                            @method('POST')
